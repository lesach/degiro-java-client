...
/// <summary>
    ...
    public class BollingerBand extends IndicatorCalculatorBase<BollingerBandSerie>
    {
        ...
        public  BollingerBandSerie Calculate()
        {
            ...
            {
                OhlcList.get(i).Close = (OhlcList.get(i).High.add(OhlcList.get(i).Low).add(OhlcList.get(i).Close)).divide(BigDecimal.valueOf(3L), MathContext.DECIMAL64);

                totalAverage = totalAverage.add(OhlcList.get(i).Close);
                totalSquares = totalSquares.add(OhlcList.get(i).Close.pow(2));
                ...
                if (i >= Period - 1)
                {
                    ...
                    BigDecimal bPercent = (OhlcList.get(i).Close.subtract(down))
                            .divide(up.subtract(down), MathContext.DECIMAL64);
                    bollingerBandSerie.BPercent.add(bPercent);

                    totalAverage = totalAverage.subtract(OhlcList.get(i - Period + 1).Close);
                    totalSquares = totalSquares.subtract(OhlcList.get(i - Period + 1).Close.pow(2));
                }
                else
                {
